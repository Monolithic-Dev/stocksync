...
def detect_cdk(project: Path) -> bool:
    """True if this looks like a CDK-deployed serverless project, in which
    case the deploy stage should run `cdk deploy`, not build/push a
    container image and kubectl-apply it."""
    if (project / "cdk.json").exists():
        return True
    # common monorepo layout: infra/cdk/cdk.json
    return any(project.glob("*/cdk/cdk.json")) or any(project.glob("*/*/cdk.json"))
...
def generate_github(stages: List[str], runtime: str, is_cdk: bool = False) -> str:
    jobs = []
    prev: List[str] = []
    for stage in stages:
        if stage == "build":
        ...
        with:
          ...
          exit-code: '1'"""
            ...
            if is_cdk:
                # No container image, no cluster — cdk deploy provisions and
                # updates the Lambda/API Gateway/DynamoDB stack directly.
                # `environment: production` is what actually gates this: set
                # a required reviewer on that GitHub Environment and this job
                # will not run until someone approves it — no custom
                # approval logic needed.
                extra = """      - name: Deploy via CDK
        run: npx cdk deploy --require-approval never --all
        working-directory: infra/cdk"""
                jobs.append(github_job("deploy", runtime, [], prev, extra=extra, environment="production"))
            else:
...
def generate_circleci(stages: List[str], runtime: str, is_cdk: bool = False) -> str:
    ...
    for stage in stages:
        ...
        elif stage == "deploy" and is_cdk:
            commands = ["npm ci --prefix infra/cdk", "npx --prefix infra/cdk cdk deploy --require-approval never --all"]
        elif stage == "deploy":
...
def main():
    ...
    is_cdk = detect_cdk(project)
    if args.verbose:
        print(f"📊 Detected runtime: {runtime}")
        if is_cdk:
            print("📊 Detected CDK project — deploy stage will run `cdk deploy`, not build/push a container image")

    if args.platform == "github":
        config = generate_github(stages, runtime, is_cdk)
        config_path = project / ".github" / "workflows" / "ci.yml"
    else:
        config = generate_circleci(stages, runtime, is_cdk)
