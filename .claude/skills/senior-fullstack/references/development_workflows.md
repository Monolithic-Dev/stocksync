...
### Serverless CDK Deploy Pipeline

For a Lambda + DynamoDB + CDK stack, there's no database migration step,
but there is a meaningful staging/production split worth keeping even on
a small team — a manual approval gate before production costs almost
nothing to set up and catches a bad deploy before it's user-facing.
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      # Fast, no-AWS-needed tests first — fail here before touching infra
      - run: npm test --workspace=packages/core
      - run: npm run build --workspace=infra/cdk
  ...
  deploy-staging:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npm ci
      - run: npx cdk deploy --context env=staging --require-approval never
        working-directory: infra/cdk

  deploy-production:
    needs: deploy-staging
    runs-on: ubuntu-latest
    environment: production  # GitHub Environments' own protection rule
                              # provides the manual approval gate here
    steps:
      - uses: actions/checkout@v4
      - run: npm ci
...
The `environment: production` line is doing the real work — configure a
required reviewer on that GitHub Environment and the job simply won't run
until someone approves it, without writing any custom approval logic.
