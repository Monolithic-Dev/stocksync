...
| Concurrent/offline multi-writer correctness (vector clocks, CRDTs, conflict resolution) | **database-designer** | When the answer to "can two clients write the same record without seeing each other's change?" is yes — this is a schema-and-correctness question, not just a schema question, so it forks alongside the normal schema fork rather than replacing it | `../../../engineering/skills/database-designer/` |
| Architecture review of this project's specific correctness guarantees (vector clocks, PN-Counter, atomic writes, FIFO grouping) | **senior-architect** | Any PR touching `packages/core`, a write-pipeline Lambda, or adding a new synced field | `../../senior-architect/` |
| Testing strategy for the sync engine (property-based tests, multi-context E2E, demo-rehearsal gate) | **senior-qa** | Before merging anything in the write pipeline; before recording the demo | `../../senior-qa/` |
| Bedrock prompt design and AI-feature guardrails | **senior-prompt-engineer** | Any new or changed Bedrock Runtime call | `../../senior-prompt-engineer/` |
| AWS service selection, cost ceiling, IAM least-privilege | **aws-solution-architect** | Before adding any new AWS resource type not already in the architecture | `../../aws-solution-architect/` |
