...
SECURITY_PATTERNS = {
    ...
    "dynamo_write_outside_transaction": {
        "pattern": r"new (?:Put|Update|Delete)(?:Item)?Command\(",
        "severity": "medium",
        "message": "Direct DynamoDB write outside an explicit transaction — if this record's state, its audit log, and its idempotency record must stay consistent, this belongs inside a TransactWriteItems call, not a standalone Put/Update/Delete (see the 'Direct Write vs Streams Race Condition' class of bug in ARCHITECTURE.md)"
    },
    "weak_idempotency_key": {
        "pattern": r"idempotency_?key\s*[:=].*(?:Date\.now\(\)|Math\.random\(\)|new Date\(\))",
        "severity": "high",
        "message": "Idempotency key derived from a timestamp or random value rather than a stable, client-generated UUID — a retried request can produce a different key each time, silently defeating deduplication"
    }
}
