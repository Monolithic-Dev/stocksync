...
- "scaffold an AWS serverless / Lambda project"
- "help me build this for a hackathon deadline"
...
Deterministic profile picker. Given four assumptions (team-size, cadence, user-facing, budget) plus optional traffic/sensitivity inputs, ranks the five built-in profiles and returns the matched profile with SLO floor and named approver chain. Refuses to recommend a profile without the four required inputs.
...
- `aws-serverless-cdk` - TypeScript monorepo: React/Vite frontend, Lambda backend (no framework — AWS Lambda Powertools instead), DynamoDB, API Gateway REST+WebSocket, CDK IaC, plus a scaffolded framework-free `packages/core` for correctness-critical logic (vector clocks + CRDT PN-Counter merge, with a working property-based test). Matches the `aws-serverless-hackathon` profile.
...
| Concurrent/offline writers on the same record | DynamoDB + vector clocks + CRDT PN-Counter (see `references/architecture_patterns.md`, "Event-Driven Conflict Resolution") |
| Fixed deadline, solo/small team, correctness over breadth | `aws-serverless-cdk` template + `aws-serverless-hackathon` profile |
...
Five built-in profiles in `profiles/` calibrate every recommendation:
...
| `aws-serverless-hackathon` | ≤ 4 eng, hard external deadline, correctness-critical core (e.g. concurrent/offline data sync) matters more than breadth | $50/mo (AWS always-free tier) | TypeScript monorepo: Lambda + DynamoDB + CDK, no backend framework, framework-free `packages/core` for the correctness-critical logic |
