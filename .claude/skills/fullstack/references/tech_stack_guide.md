...
### AWS Serverless (Lambda + API Gateway + DynamoDB), in more depth

**Best for:** Solo/small-team builds under a fixed deadline or budget,
correctness-critical backends with modest traffic, anything that should
scale to zero when idle. This is a distinct pattern from "AWS" generally —
worth its own entry because "use AWS" says almost nothing about whether
you're running Kubernetes or five Lambda functions, and the two imply
completely different team sizes and operational burden.

**No backend framework, by design:** Express/NestJS/Fastify solve
problems specific to a long-lived server process — routing, middleware
chains, process lifecycle. API Gateway already routes requests to
specific Lambda invocations, and there's no process to keep alive. Adding
a full framework here means carrying its weight to solve a problem this
architecture doesn't have. Use a thin middleware toolkit instead:

```typescript
// AWS Lambda Powertools — structured logging, idempotency, metrics,
// without the weight of a full framework
import { Logger } from "@aws-lambda-powertools/logger";
import { makeIdempotent } from "@aws-lambda-powertools/idempotency";
import { DynamoDBPersistenceLayer } from "@aws-lambda-powertools/idempotency/dynamodb";

const logger = new Logger({ serviceName: "write-intake" });
const persistenceStore = new DynamoDBPersistenceLayer({ tableName: "write_dedup" });

export const handler = makeIdempotent(
  async (event) => {
    logger.info("processing write", { requestId: event.requestContext.requestId });
    // ... handler logic, no framework needed
  },
  { persistenceStore }
);
```

**DynamoDB Streams + SQS FIFO for ordered, event-driven writes:**

```typescript
// SQS FIFO MessageGroupId should be the RESOURCE being modified,
// not the sender — grouping by sender lets two concurrent senders
// race on the same resource; grouping by resource serializes them.
await sqs.send(new SendMessageCommand({
  QueueUrl: queueUrl,
  MessageBody: JSON.stringify(transaction),
  MessageGroupId: transaction.recordId,       // not transaction.clientId
  MessageDeduplicationId: transaction.idempotencyKey,
}));
```

**Atomic cross-table writes with `TransactWriteItems`:**

```typescript
// When a resolved write must update live state, an audit log, AND an
// idempotency record together, do it as one atomic operation — not
// three separate calls that can leave the system half-updated on a crash.
await ddb.send(new TransactWriteCommand({
  TransactItems: [
    { Update: { TableName: "records", Key: { pk, sk }, /* ... */ } },
    { Put: { TableName: "audit_log", Item: auditEntry } },
    { Put: { TableName: "write_dedup", Item: dedupRecord } },
  ],
}));
```

**CDK over hand-written SAM/CloudFormation, when the rest of the stack is
already TypeScript:**

```typescript
// infra/cdk/lib/stack.ts
const table = new dynamodb.TableV2(this, "Records", {
  partitionKey: { name: "pk", type: dynamodb.AttributeType.STRING },
  sortKey: { name: "sk", type: dynamodb.AttributeType.STRING },
  billing: dynamodb.Billing.onDemand(),
});

const resolverFn = new NodejsFunction(this, "ConflictResolver", {
  entry: "../../apps/api/src/handlers/conflictResolver.ts",
  runtime: lambda.Runtime.NODEJS_20_X,
});
table.grantWriteData(resolverFn); // least-privilege, not a blanket policy
```

| Pattern | Use when | Skip when |
|---|---|---|
| No framework + Lambda Powertools | Solo/small team, API Gateway already routes, cadence is "ship it" not "iterate on a long-lived service" | You're running the same app on a persistent server elsewhere too and want shared middleware code across both |
| DynamoDB Streams + SQS FIFO grouped by resource | Any workload where concurrent writers can touch the same record and ordering/atomicity matters | Single-writer workloads with no concurrency to reconcile |
| CDK (TypeScript) | Monorepo already in TypeScript; want type-checked infrastructure | Team already fluent in SAM/Terraform and doesn't want a second IaC tool to learn |
| Bedrock as a narrow, async, non-blocking call | A specific, bounded feature (explain a conflict, summarize data) where a wrong/slow answer doesn't break core correctness | Anything on the critical path where the system's core guarantee must hold even if the AI call fails or times out |

**Canon:** AWS Well-Architected Framework, Serverless Applications Lens
(2023); Shapiro, Preguica, Baquero & Zawirski, "Conflict-free Replicated
Data Types" (INRIA, 2011) for the ordering/merge reasoning above.
