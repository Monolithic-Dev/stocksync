...
export function ConnectivityToggle({ isOnline, onToggle }: ConnectivityToggleProps) {
  return (
    <button
      ...
      className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold shadow-sm transition-colors ${
        isOnline ? "bg-emerald-600 text-white hover:bg-emerald-700" : "bg-rose-600 text-white hover:bg-rose-700"
      }`}
    >
      <span className={`h-2.5 w-2.5 rounded-full ${isOnline ? "bg-white" : "bg-white animate-pulse"}`} />
