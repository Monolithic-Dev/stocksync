...
export function CartDrawer({ lineItems, productName, onRemove }: CartDrawerProps) {
  ...
  if (lineItems.length === 0) {
    return <p className="text-sm text-slate-400 dark:text-slate-500">Cart is empty — add items from the list.</p>;
  }
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div
        className="mt-2 flex justify-between border-t border-slate-200 pt-2 text-sm font-semibold text-slate-900 dark:border-slate-800 dark:text-slate-100"
