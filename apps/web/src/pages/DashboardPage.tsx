...
function trustScoreTone(percent: number): { text: string; ring: string } {
  if (percent >= 90)
    return { text: "text-emerald-700 dark:text-emerald-400", ring: "border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/40" };
  if (percent >= 70)
    return { text: "text-amber-700 dark:text-amber-400", ring: "border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/40" };
  return { text: "text-rose-700 dark:text-rose-400", ring: "border-rose-200 bg-rose-50 dark:border-rose-900 dark:bg-rose-950/40" };
}
...
export function DashboardPage({ shopId }: DashboardPageProps) {
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-3xl p-4 sm:p-6">
        <p className="text-sm text-rose-600 dark:text-rose-400">Couldn't load the dashboard. Check the connection and try again.</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="mx-auto max-w-3xl p-4 sm:p-6">
        <h1 className="mb-4 text-xl font-bold text-slate-900 dark:text-slate-100">Dashboard</h1>
        <DashboardSkeleton />
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl p-4 sm:p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-1 text-2xl font-bold text-slate-900 dark:text-slate-100">₹{data.revenue.total.toLocaleString()}</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Last 7 days</p>
          <p className="mt-1 text-2xl font-bold text-slate-900 dark:text-slate-100">₹{data.revenue.last_7_days.toLocaleString()}</p>
        </div>

        <div className={`rounded-xl border p-4 ${tone.ring}`}>
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Trust score</p>
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        ...
        {data.low_stock.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Nothing running low right now.</p>
        ) : (
            ...
            {data.low_stock.map((item) => (
              <li key={item.item_id} className="flex items-center justify-between py-2 text-sm">
                <span className="flex items-center gap-1.5 text-slate-800 dark:text-slate-300">
                  <TagIcon className="h-3.5 w-3.5 text-slate-400 dark:text-slate-500" />
                  {item.name ?? item.item_id}
                </span>
                <span className={item.stock === 0 ? "font-semibold text-rose-600 dark:text-rose-400" : "text-amber-600 dark:text-amber-400"}>
                  {item.stock} left
                </span>
              </li>
            ))}
