...
| FR-24 | For any resolved audit-log entry, the client SHALL be able to display the actual vector clocks compared (stored vs. incoming) and a plain-language explanation of the resulting decision (clean apply, merged, or needs review) — a read-only view over `audit_log` data, never a second place resolution logic lives. |
