...
| C-6 | Two devices' clocks are skewed relative to each other (e.g. Counter B's clock is a few minutes behind Counter A's) when both are used to compute `overlap_seconds` for a flagged conflict | The computed `overlap_seconds` may be inaccurate, or in extreme skew even negative. This is acceptable *only* because `overlap_seconds` is advisory context for a human/Bedrock explanation, never an input to `resolve()`'s actual decision (which uses vector clocks exclusively — see C-4). Clamp negative values to 0 before display, and never surface `overlap_seconds` with more apparent precision than it deserves. |
