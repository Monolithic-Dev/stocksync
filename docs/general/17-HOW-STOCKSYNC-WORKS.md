...
**The cross-account workaround (read this — it's unusual).** The account
this stack deploys into is blocked from invoking *any* Anthropic model on
Bedrock by an account-level "Anthropic use-case details" approval gate;
clearing it needs a support case, which has no guaranteed resolution time.
So the conflict-resolver calls Bedrock through a **second, unrestricted AWS
account** instead:

1. That account's access key is stored in **AWS Secrets Manager** (see
   4.6), created empty by CDK and filled in out-of-band with
   `aws secretsmanager put-secret-value` — the key is never in source
   control.
2. At runtime `apps/api/src/lib/bedrock.ts` reads the secret (cached after
   the first read), builds a `BedrockRuntimeClient` with those credentials,
   and invokes a cross-account **inference profile**
   (`BEDROCK_INFERENCE_PROFILE_ARN`).
3. The resolver's own `bedrock:InvokeModel` grant is kept anyway, so if
   the deployment account's approval ever clears, deleting the two env
   vars reverts to same-account behaviour with no code change.

This is safe *because* Bedrock is advisory and non-blocking: if the secret
is missing, the second account is unreachable, or anything else fails, the
explanation is simply absent and the conflict still works. It is a
workaround, not a design goal — treat it as temporary.

### 4.6 AWS Secrets Manager — holding the one credential we have

**What it is:** managed, encrypted storage for secrets, with fine-grained
IAM access and audit logging.

**What we use it for:** exactly one secret,
`BedrockCrossAccountCredentials` — the access key for the second AWS
account that serves Bedrock calls (see 4.5). CDK creates the secret
*resource* and grants the conflict-resolver **read-only access to that one
secret**; the actual key is put there by hand with the AWS CLI so it never
appears in Git, CloudFormation templates, or Lambda environment variables
(env vars would be visible in the console and in `describe-function`).

**Why not just an env var or a committed file:** an access key in either
place is one screenshot or one `git add .` away from leaking. Secrets
Manager keeps it out of the code path entirely and lets it be rotated
without a redeploy.

**What breaks without it:** only the Bedrock explanation — nothing else in
the system touches this secret.

**Cost note:** unlike almost everything else here, Secrets Manager is
**not** free-tier — roughly $0.40 per secret per month plus a tiny per-call
fee. It's one secret, so it's pennies, but it is a real line item.

### 4.7 Amazon CloudWatch — operating it
...
asserts this. The resolver's Secrets Manager access is read-only on the one
Bedrock-credentials secret, and the catalog CRUD Lambdas cannot touch
`inventory_records` or `audit_log` at all (also asserted by a CDK test).

### 4.9 AWS CDK / CloudFormation — infrastructure as code
...
### 4.10 AWS Amplify Hosting — the frontend (separate deploy)
...
### 4.11 Services we deliberately do NOT use (yet)
...
At demo scale almost everything sits inside AWS's always-free
...
negligible — though heavy rehearsal is worth a glance at the bill. The
one exception is Secrets Manager (about $0.40/month for its one secret).
Set a
...
- **`overlap_seconds` is approximate** — derived from client clocks, which
  ...
  the resolver.
- **The live stack predates Phase 19b.** The backend and frontend are deployed, but the four Tier-2 tables, four new Lambdas, and the Products/Checkout pages need a `cdk deploy` plus an Amplify re-upload before they exist live. Bedrock explanations also depend on the cross-account workaround in 4.5 being provisioned.
